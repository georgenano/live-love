...
<body>
	<p>
		<a href="${f:url('../logout')}"
			onclick="return confirm('ログアウトしていいですか？')">ログアウト</a>
	</p>

	<p>ライブ情報登録</p>
	<form method="post" action="registLiveReport">
		<table summary="ライブ情報記入フォーム">
			<tr>
				<th><label for="liveName">ライブ名</label></th>
				<td><input type="text" name="liveName" id="liveName" value=""
					size="20" class="required" title="入力してください" /></td>
			</tr>
			<tr>
				<th><label for="artistList">出演アーティスト</label></th>
				<td><input type="text" name="artistList" id="artistList"
					value="" size="20" class="required" title="入力してください" /></td>
			</tr>
			<tr>
				<th>地域</th>
			...
			<tr>
				...
				<td><input type="text" name="livePlace" id="livePlace" value=""
					size="10" class="required" title="入力してください" /></td>
			</tr>
			<tr>
				<th><label for="start_year">開催日</label></th>
	...
	<table>
		...
		<c:forEach var="e" items="${liveReportList}">
			<tr>
				<td>${f:h(e.registUser)}</td>
				<td>${f:h(e.note)}</td>
				<td><fmt:formatDate value="${e.registDate}"
						pattern="yyyy-MM-dd" timeZone="JST" /></td>
				<c:set var="deleteUrl"
					value="delete?key=${f:h(e.key)}&amp;version=${e.version}" />
				<td><a href="${f:url(deleteUrl)}"
					onclick="return confirm('削除していいですか？')">削除</a></td>
			</tr>
		</c:forEach>
	</table>
</body>
